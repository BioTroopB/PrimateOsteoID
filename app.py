...
if uploaded_file is not None:
    ...
    # FIXED AUTO-DETECTION (tuned to real .ply vertex counts)
    if bone == "Auto":
        n_verts = len(verts)
        if n_verts < 15_000:
            bone = "clavicle"
    ...
    # FIXED ICP — proper unpacking of procrustes (disp is scalar now)
    def simple_icp(source, target, max_iterations=30, threshold=1e-6):
        ...
        for _ in range(max_iterations):
            ...
            # FIXED: Unpack as _, s, disp — s gets transformed, disp is scalar disparity
    ...
    # Sample points for speed (match your landmark count per bone)
    landmark_counts = {"clavicle": 7, "scapula": 13, "humerus": 16}
    n_samples = landmark_counts.get(bone, 10)
    sample_idx = np.random.choice(len(verts), size=min(n_samples * 10, len(verts)), replace=False)  # Oversample for better ICP
